...
void dmaPioInit()
{
    // DMA Read  = SD  -> Mem
    // DMA Write = Mem -> SD
    int romProgram = pio_add_program(FIFO_PIO, &rom_program);
    pio_sm_clear_fifos(FIFO_PIO, dmaRomSm);

    pio_sm_config romConfig = rom_program_get_default_config(romProgram);
    ...
    sm_config_set_in_shift(&romConfig, false, false, 32); // L shift
    sm_config_set_out_shift(&romConfig, true, false, 32); // R shift
    sm_config_set_clkdiv(&romConfig, 1.0f);

    pio_sm_init(FIFO_PIO, dmaRomSm, romProgram, &romConfig);
