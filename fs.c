...
BYTE fs_init()
{
  ...
  while (1)
  {
    ...
    {
      if (LD_WORD(buf + BS_FilSysType) == 0x4146)
        break;
  ...
  fs_fatbase2 = 0;
  if (buf[BPB_NumFATs] >= 2)
    fs_fatbase2 = fs_fatbase + fsize;
...
static DWORD fs_getEntryCluster()
{
  ...
  if (fs_type != FS_FAT16)
    c |= ((DWORD)LD_WORD(FS_DIRENTRY + DIR_FstClusHI)) << 16;
  return c;
}
...
static BYTE fs_nextCluster()
{
  if (fs_type == FS_FAT16)
  {
    if (sd_read((BYTE *)&fs_tmp, fs_fatbase + fs_tmp / 256, (WORD)(BYTE)fs_tmp * 2, 2))
      goto abort;
    fs_tmp &= 0xFFFF;
  }
  else
  {
    if (sd_read((BYTE *)&fs_tmp, fs_fatbase + fs_tmp / 128, (WORD)((BYTE)fs_tmp % 128) * 4, 4))
      goto abort;
    fs_tmp &= 0x0FFFFFFF;
  }
...
static BYTE fs_readdirInt()
{
    ...
    if (fs_file.entry_index % 16 == 0)
    {
      ...
      if (fs_file.entry_cluster != 0 && ((fs_file.entry_index / 16) % fs_csize) == 0)
      {
        ...
        if (fs_nextCluster())
          return 1;
        if (fs_tmp == 0)
          goto retEnd; /* Последний кластер, устаналиваем fs_file.entry_able = 0 */
  ...
  else
  {
    ...
    if (fs_file.entry_cluster == 0)
      fs_file.entry_sector = fs_dirbase;
  }
...
BYTE fs_readdir_nocheck()
{
  while (!fs_readdirInt())
  {
    if (FS_DIRENTRY[DIR_Name] == 0)
      fs_file.entry_able = 0;
    if (fs_file.entry_able == 0)
      return 0;
    if (FS_DIRENTRY[DIR_Name] == 0xE5)
      continue; /*  Может быть еще 0x05 */
    if (FS_DIRENTRY[DIR_Name] == '.')
      continue;
    if ((FS_DIRENTRY[DIR_Attr] & AM_VOL) == 0)
      return 0;
  }
  return 1;
}
...
{
  if (fs_fatbase2)
    if (sd_writeBuf(fs_fatbase2 + sector))
      return 1;
...
static BYTE fs_allocCluster(DIS(BYTE freeSpace))
{
  ...
  while (1)
  {
    ...
    if (fs_type != FS_FAT16)
      s = fs_tmp / 128, i |= 128, a = (BYTE *)((DWORD *)buf - 128 + i);

    /* Читаем сектор */
    if (sd_readBuf(fs_fatbase + s))
      goto abort;
    ...
    {
      /* Кластеры кончились */
      if (fs_tmp >= fs_n_fatent)
      {
        DIS(if (freeSpace) return 0;)
        lastError = ERR_NO_FREE_SPACE;
        goto abort;
      }
      ...
      else
      {
        if (LD_DWORD(a) == 0)
        {
          DIS(if (!freeSpace) {) LD_DWORD(a) = LAST_CLUSTER; goto founded; DIS( } fs_file.sector++;)
        }
        a += 4;
      }
...
static BYTE fs_setNextCluster(DWORD cluster)
{
  ...
  if (fs_tmp == FREE_CLUSTER)
    if (cluster < fs_fatoptim)
      fs_fatoptim = cluster;
...
static BYTE *fs_allocEntry()
{
  ...
  while (1)
  {
    ...
    if (FS_DIRENTRY[0] == 0xE5 || FS_DIRENTRY[0] == 0)
    { /* Может быть еще 0x05 */

      /* Читаем сектор */
      if (sd_readBuf(fs_file.entry_sector))
        return 0;
  ...
  if (fs_file.entry_index == 0)
  {
    lastError = ERR_DIR_FULL;
    return 0;
  }

  /* Выделить кластер. Результат в fs_tmp */
  if (fs_allocCluster(ALLOCCLUSTER))
    return 0;

  /* Добавить еще один кластер к папке. */
  if (fs_setNextCluster(fs_file.entry_cluster))
    return 0; /* fs_tmp сохранится, так как он не LAST и не FREE */
...
BYTE fs_open0(BYTE what)
{
  ...
  else
  {
    ...
    while (1)
    {
      ...
      if (path == (CONST BYTE *)1)
        goto abort_noPath;
      /* Ищем имя в папке */
      fs_file.entry_able = 0;
      while (1)
      {
        if (fs_readdir_nocheck())
          return 1;
        if (fs_file.entry_able == 0)
          break;
        if (!memcmp(FS_DIRENTRY, FS_DIRFIND, 11))
          break;
      }
      ...
      if (what && path == 0)
      {
        ...
        if (fs_type == FS_FAT32 && fs_parent_dir_cluster == fs_dirbase)
          fs_parent_dir_cluster = 0;
        if (fs_file.entry_able == 0)
          return fs_open0_create(what - 1); /* Продолжим там */
      ...
      if ((FS_DIRENTRY[DIR_Attr] & AM_DIR) == 0)
        goto abort_noPath;
      /* Предотвращаем рекурсию для функции fs_move */
      if (r && fs_file.entry_start_cluster == fs_parent_dir_cluster)
        goto abort_noPath;
...
static BYTE exists(const BYTE *str, BYTE c)
{
  while (*str)
    if (*str++ == c)
      return c;
...
static BYTE fs_open0_create(BYTE dir)
{
  ...
  {
    if (fs_allocCluster(ALLOCCLUSTER))
      goto abort; /* fs_file.entry_start_cluster изменен не будет, там первый кластер папки в которой мы создадим файл */
  ...
  if (!dir)
  {
    ...
    if (sd_writeBuf(fs_file.entry_sector))
      goto abort;
  ...
  buf[32 + 11] = 0x10;
  if (fs_notrootdir)
    fs_setEntryCluster(buf + 32, fs_file.entry_start_cluster); /* Сейчас в fs_file.size==0 значит корневой каталог */
...
static BYTE fs_nextRWSector()
{
  ...
  else
  {
    ...
    if (((fs_file.sector - fs_database) % fs_csize) != 0)
      return 0;
  ...
  if (fs_tmp == 0)
  {
    if (fs_allocCluster(ALLOCCLUSTER))
      return 1;
    if (fs_file.ptr == 0)
      fs_file.entry_start_cluster = fs_tmp;
    else
      fs_setNextCluster(fs_file.cluster); /* fs_tmp сохранится, так как он не LAST и не FREE */
  }
...
BYTE fs_read0(BYTE *ptr, WORD len)
{
  ...
  while (len)
  {
    ...
    if (ptr)
    {
      if (sd_read(ptr, fs_file.sector, (WORD)fs_file.ptr % 512, sectorLen))
        goto abort;
      ptr += sectorLen;
    }
  ...
  if (fs_file.ptr > fs_file.size)
    fs_file.size = fs_file.ptr, fs_file.changed = 1;
...
BYTE fs_lseek(DWORD off, BYTE mode)
{
  ...
  do
  { /* Выполняем один цикл даже для off=0, так как внутри происходит проверка на ошибки */
    l = off;
    if (l > LSEEK_STEP)
      l = LSEEK_STEP;
    if (fs_read0(0, (WORD)l))
      return 1;
    off -= l;
  } while (off);
...
BYTE fs_write_start()
{
  ...
  if (fs_nextRWSector())
    goto abort; /* Должно вылетать только по ошибкам ERR_NO_FREE_SPACE, ERR_DISK_ERR */
...
static BYTE fs_freeChain()
{
  ...
  while (1)
  {
    ...
    if (fs_setNextCluster(c))
      break; /* fs_tmp будет содержать следующий кластер, так как записывается FREE_CLUSTER */
  }
  return 1;
}
...
BYTE fs_move0()
{
  ...
  if (sd_readBuf(old_sector))
    goto abort;
  entry = buf + (old_index % 16) * 32;
  memcpy(tmp, entry + 11, 21);
  entry[0] = 0xE5;
  if (sd_writeBuf(old_sector))
    goto abort;
...
BYTE fs_delete()
{
  ...
  if (FS_DIRENTRY[DIR_Attr] & AM_DIR)
  {
    ...
    if (fs_file.entry_able)
    {
      lastError = ERR_DIR_NOT_EMPTY;
      goto abort;
    }
  }

  /* Удаляем описатель */
  if (sd_readBuf(entrySector))
    goto abort;
  entry[0] = 0xE5;
  if (sd_writeBuf(entrySector))
    goto abort;
...
BYTE fs_write_eof()
{
  ...
  else
  {
    ...
    fs_tmp = LAST_CLUSTER;
    if (fs_setNextCluster(fs_file.cluster))
      goto abort; /* fs_tmp будет содержать следующий кластер, так как записывается LAST_CLUSTER */
  }
