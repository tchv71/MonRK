...
int main()
{
    ...
    retval = socket((uint8_t)(uint32_t)(g_ssl.private_p_bio), Sn_MR_TCP, PORT_SSL, SF_TCP_NODELAY);
    ...
    do
    {
        retval = connect((uint8_t)(uint32_t)(g_ssl.private_p_bio), g_ssl_target_ip, PORT_SSL);
    ...
    while (1)
    {
        getsockopt((uint8_t)(uint32_t)(g_ssl.private_p_bio), SO_RECVBUF, &len);
...
static int recv_timeout(void *ctx, unsigned char *buf, size_t len, uint32_t timeout)
{
    ...
    do
    {
        getsockopt((uint8_t)(uint32_t)(ctx), SO_RECVBUF, &recv_len);

        if (recv_len > 0)
        {
            return recv((uint8_t)(uint32_t)ctx, (uint8_t *)buf, (uint16_t)len);
        }
